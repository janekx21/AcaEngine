...
namespace graphics {
	...
	FontRenderer::FontRenderer() : m_texture(nullptr) {
...
#pragma pack(pop)
	...
	void FontRenderer::storeCaf(const char *_fontFile) {
		...
		if (!file) {
			spdlog::error("[graphics] Cannot open file ", _fontFile, " for writing!");
			return;
		}
	...
	void FontRenderer::loadCaf(const char *_fontFile) {
		...
		if (!file) {
			spdlog::error("[graphics] Cannot open file ", _fontFile, " for reading!");
			return;
		}
