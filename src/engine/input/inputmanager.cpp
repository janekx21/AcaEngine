...
namespace input {
	...
	void InputManager::keyCallback(GLFWwindow *window, int key, int scancode, int action, int mods) {
		switch (action) {
			case GLFW_PRESS:
				m_keyStates[key] = ActionState::PRESSED;
				break;
			case GLFW_RELEASE:
				m_keyStates[key] = ActionState::RELEASED;
				break;
		}
	}
	...
	void InputManager::buttonCallback(GLFWwindow *window, int button, int action, int mods) {
		switch (action) {
			case GLFW_PRESS:
				m_buttonStates[button] = ActionState::PRESSED;
				break;
			case GLFW_RELEASE:
				m_buttonStates[button] = ActionState::RELEASED;
				break;
		}
	}
	...
	glm::vec2 InputManager::getCursorPos() { return cursorPosition; }
