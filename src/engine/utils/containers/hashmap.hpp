...
namespace utils {
	...
	public:
		...
		class HandleT {
			...
			HandleT(MapT *_map, uint32_t _idx = 0) : map(_map),
																							 idx(_idx) {
				if (_map) {
					if (_map->m_size == 0)
						map = nullptr;
		...
		explicit HashMap(uint32_t _expectedElementCount = 15) : m_capacity(estimateCapacity(_expectedElementCount)),
																														m_size(0) {
		...
		HashMap(HashMap &&_other) : m_capacity(_other.m_capacity),
