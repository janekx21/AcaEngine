...
game::HorizontalSpring::HorizontalSpring() : game::GameState(),
	...
	for (int i = 0; i < 2000; i++) {
		...
		registry.addComponent<Visibility>(entities[i], true);
		registry.addComponent<Mesh>(entities[i], _mesh);
		registry.addComponent<Texture>(entities[i], texture);
		registry.addComponent<Transform>(entities[i], glm::identity<glm::quat>(), glm::vec3(1, 1, 1), glm::vec3(5, 0, -i));
		registry.addComponent<Velocity>(entities[i], glm::vec3(i, 0, 0));
	}
